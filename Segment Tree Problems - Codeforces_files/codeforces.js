...
//            window.localStorage[key] = defaultValue;
    ...
    this.filterClipboardText = function(text) { // Because of unexpected bug in prettyprint or Clipboard lib.
        var result = "";
        for (var i = 0; i < text.length; i++) {
            var cur = text.charAt(i).charCodeAt(0);
            if (cur === 160 && i > 0 && i + 1 < text.length) {
                var prev = text.charAt(i - 1).charCodeAt(0);
                var next = text.charAt(i + 1).charCodeAt(0);
